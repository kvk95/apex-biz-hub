...
export default function VariantAttributes() {
  ...
  const [form, setForm] = useState<VariantRecord>({
    id: 0,
    variantName: "",
    variantValue: "",
  ...
  useEffect(() => {
    const loadData = async () => {
      setLoading(true);
      const response = await apiService.get<VariantRecord[]>("VariantAttributes");
  ...
  const filteredData = useMemo(() => {
    return data.filter((item) => {
      const matchesSearch =
        ...
        item.variantValue.toLowerCase().includes(searchText.toLowerCase());
  ...
  const handleFormSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.variantName.trim() || !form.variantValue.trim() || !form.status) {
  ...
  const handleEdit = (record: VariantRecord) => {
    setForm(record);
    setFormMode("edit");
  };
  ...
  const handleReport = () => {
    alert("Variant Attributes Report:\n\n" + JSON.stringify(filteredData, null, 2));
  };
  ...
  const columns: Column[] = [
    ...
    { key: "variantValue", label: "Variant Value", align: "left" },
    { key: "status", label: "Status", align: "center", render: renderStatusBadge },
  ];

  const rowActions = (row: VariantRecord) => (
    <>
      <button
        onClick={() => handleEdit(row)}
        aria-label={`Edit ${row.variantName}`}
      ...
      <button
        ...
        aria-label={`Delete ${row.variantName}`}
  ...
  const customFilters = () => (
    <div className="flex flex-wrap gap-2 mb-4">
      <input
        ...
        aria-label="Search by variant name or value"
      />
      <select
        value={filterStatus}
        onChange={(e) => {
  ...
  const modalForm = () => (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div>
        <label htmlFor="variantName" className="block text-sm font-medium mb-1">
        ...
        <input
          ...
          required
          aria-label="Enter variant name"
        />
      </div>
      <div>
        <label htmlFor="variantValue" className="block text-sm font-medium mb-1">
        ...
        <input
          ...
          required
          aria-label="Enter variant value"
        />
      </div>
      <div>
