...
const IncomeCategory: React.FC = () => {
  ...
  const loadData = async () => {
    setLoading(true);
    const response = await apiService.get<IncomeCategoryItem[]>(
      "IncomeCategory"
    );
  ...
  const filteredData = data.filter((item) => {
    const matchesSearch =
      !searchTerm.trim() ||
      item.incomeCategory.toLowerCase().includes(searchTerm.toLowerCase());
    const matchesStatus =
      filterStatus === "All" || item.status === filterStatus;
    const matchesCategory =
      !filterCategory || item.incomeCategory === filterCategory;
  ...
  return (
    <PageBase1
      ...
      modalTitle={
        formMode === "add" ? "Add Income Category" : "Edit Income Category"
      }
      modalForm={modalForm}
